...
export class OpenAPIPlugin {
	...
	private processHandleCall(
		...
		this.builder.addOperation({
			...
			summary:
				(openapiConfig.summary as string | undefined) ??
			...
			request: openapiConfig.request as unknown as RouteSchemas,
			responses: openapiConfig.responses as unknown as ResponsesMap,
			filePath: fileName,
		});
	}
	...
	private extractObjectLiteralValue(
		...
		switch (methodName) {
			...
			case "array":
				if (args.length > 0) {
					...
					return {
						type: "array",
						items: (itemSchema as Record<string, unknown>) || {
							type: "unknown",
						},
					};
				}
