...
type VSchemaLike = v.BaseSchema<unknown, unknown, v.BaseIssue<unknown>> & {
    ...
    entries?: Record<string, v.BaseSchema<unknown, unknown, v.BaseIssue<unknown>> & VSchemaLike>;
    options?: Array<v.BaseSchema<unknown, unknown, v.BaseIssue<unknown>> & VSchemaLike>;
    value?: unknown;
    isOptional?: boolean;
};
	...
	switch (type) {
        ...
        case "object": {
            ...
            for (const [k, sch] of Object.entries(entries ?? {})) {
                ...
                if (!(sch as VSchemaLike).isOptional) required.push(k);
            }
            const base: Record<string, unknown> = { type: "object", properties };
            if (required.length) (base as { required?: string[] }).required = required;
            return base as Json;
        }
        case "union": {
        ...
        case "literal":
            return { const: (s as VSchemaLike).value } as unknown as Json;
        case "unknown":
            return {};
        default:
...
type RequestBodyObject = { required?: boolean; content: Record<string, MediaTypeObject> };
type ResponseObject = {
    description?: string;
    headers?: Record<string, { description?: string; schema?: Json } | ReferenceObject>;
    ...
    private components: ComponentsObject = {
        ...
        parameters: {
            ...
            PageSize: {
                ...
                in: "query",
                required: false,
                schema: { type: "integer", minimum: 1, maximum: 200 } as unknown as Json,
